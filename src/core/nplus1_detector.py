...
_query_context: ContextVar[Optional["QueryContext"]] = ContextVar("query_context", default=None)
...
class QueryContext:
    ...
    model_access: Dict[str, List[str]] = field(default_factory=lambda: defaultdict(list))
...
class BatchLoader:
    ...
    async def load(
        ...
        loader_key: str,
        key: Any,
        load_func: Callable[[List[Any]], Any]
    ) -> Any:
...
class RelationshipPrefetch:
    ...
    async def load(
        ...
        chunks = [list(to_load)[i:i + batch_size] for i in range(0, len(to_load), batch_size)]
