...
from typing import (
    Any, Callable, Dict, Generic, List, Optional, Protocol, Set, TypeVar,
    Iterator, AsyncIterator, Union
)
...
class Tracer:
    ...
    def _track_cost(self, span: Span) -> None:
        ...
        if tenant_id:
            self._add_cost_to_breakdown(
                self._cost_by_tenant[tenant_id],
                span.cost
            )
        
        if user_id:
            self._add_cost_to_breakdown(
                self._cost_by_user[user_id],
                span.cost
            )
        ...
        def decorator(func: Callable) -> Callable:
            ...
            async def async_wrapper(*args, **kwargs):
                with self.span(span_name, kind) as span:
                    ...
                    # Add compute cost
                    if self.config.enable_cost_tracking:
                        compute_cost = Decimal(str(duration)) * self.config.cost_per_compute_second
