...
async def transaction(
    ...
    try:
        ...
        if isolation_level:
            await db_session.execute(f"SET TRANSACTION ISOLATION LEVEL {isolation_level.value}")
...
def transactional(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            async with transaction(isolation_level, retry_config=retry_config) as session:
                # Inject session as first argument if not provided
                if 'session' not in kwargs and not any(isinstance(arg, AsyncSession) for arg in args):
