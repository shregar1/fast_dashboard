...
class TestModelEncryption:
    ...
    def test_detect_encrypted_fields(self):
        ...
        class MockModel:
            """Represents the MockModel class."""
    ...
    def test_is_encrypted_detection(self):
        ...
        assert (
            model_encryption._is_encrypted(f"{FieldEncryption.VERSION}:key-1:abc123")
            is True
        )
...
class TestSetupEncryption:
    ...
    @patch.dict(
        "os.environ",
        {"FASTMVC_ENCRYPTION_KEY": base64.urlsafe_b64encode(b"x" * 32).decode()},
    )
